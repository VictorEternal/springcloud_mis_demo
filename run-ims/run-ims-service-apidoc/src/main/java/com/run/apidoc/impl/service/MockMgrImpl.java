...
import com.run.apidoc.api.service.MockMgr;
import com.run.apidoc.api.service.ProjectMgr;
import com.run.apidoc.constant.Patterns;
import com.run.apidoc.constant.SystemSettings;
...
import com.run.apidoc.utils.FileUtils;
import com.run.apidoc.utils.JSRunner;
import com.run.apidoc.utils.MockjsRunner;
import com.run.apidoc.utils.NumberUtils;
import com.run.apidoc.utils.StringUtils;
import com.run.apidoc.utils.URLUtils;
...
public class MockMgrImpl implements MockMgr {
	 ...
	 */
	private int							_num			= 1;
	private String[]					NAME_LIB		= { "霍雍", "行列", "幻刺", "金台", "望天", "李牧", "三冰", "自勉", "思霏", "诚冉",
			"甘苦", "勇智", "墨汁老湿", "圣香", "定球", "征宇", "灵兮", "永盛", "小婉", "紫丞", "少侠", "木谦", "周亮", "宝山", "张中", "晓哲", "夜沨" };
	...
	public String generateData(int projectId, String pattern, Map<String, Object> options)
		...
		List<Action> aList = projectMgr.getMatchedActionList(projectId, pattern);
		if (aList.size() == 0)
	...
	public String generateRuleData(final int projectId, String pattern, Map<String, Object> options)
			throws UnsupportedEncodingException {
		...
		if (returnValue == null) {
			return "{\"isOk\":false, \"errMsg\":\"运行超时," + SystemSettings.MOCK_SERVICE_TIMEOUT
					+ "毫秒都跑不完,服务器HOLD不住啊,亲的自定义函数是不是写的太夸张了啊...\"}";
		} else {
	...
	public String validateAPI(int projectId, String pattern, Map<String, Object> options, String jsonToCompare)
		...
		jsCode.append(FileUtils.readFileContent(FileUtils.JS_UTIL_DIR_PATH + "mock-min.js"))
				.append(FileUtils.readFileContent(FileUtils.JS_UTIL_DIR_PATH + "structureValidator.js"))
				.append("var o1 = ").append(mockjsData).append(";\n").append("var o2 = ").append(jsonToCompare)
	...
	public String generateRule(int projectId, String pattern, Map<String, Object> options)
			throws UnsupportedEncodingException {
		if (!isPatternLegal(pattern)) {
			return ERROR_PATTERN;
		}
	...
	private Action actionPick(List<Action> actionList, String pattern, Map<String, Object> options)
			throws UnsupportedEncodingException {
		...
		for (Action action : actionList) {
			if (options.get("method") != null && action.getMethod().equals(options.get("method").toString())) {
				result = action;
				break;
			}
		}
		...
		// schema match
		for (Action action : actionList) {
			Map<String, List<String>> docActionParams = getUrlParameters(action.getRequestUrl());
	...
	private void buildJson(StringBuilder json, Parameter para, int index) {
		...
		if (para.getDataType().contains("array") && tagMap.get("value") != null) {
			...
			if (para.getParameterList() == null || para.getParameterList().size() == 0) {
	...
	private void buildMockTemplate(StringBuilder json, Parameter para, int index) {
		...
		if (para.getParameterList() == null || para.getParameterList().size() == 0 || para.hasMockJSData()) {
	...
	private String mockjsValue(Parameter para, int index) {
		...
		if (mockValue != null && !mockValue.isEmpty()) {
			if (mockValue.startsWith("[") && mockValue.endsWith("]")) {
				...
				if (para.getDataType().contains("array")) {
					...
					orderCmdFunc.append("function() {").append("   var window = function(){return this;}();")
							.append("	function geneVal(key) {").append("		var o = __rap__context__[key];")
							.append("		var arr = o.arr;").append("		return arr[o.index++ % arr.length];")
							.append("	}").append("	if (!window.__rap__context__) {")
							.append("		window.__rap__context__ = {};").append("	}")
							.append("	var orderCmd = \"" + StringUtils.escapeInJ(mockValue) + "\";")
							.append("	var orderArr = eval('[' + orderCmd.substring(7, orderCmd.length - 1) + ']');")
							.append("	var key = '" + (uid++) + "';").append("	if (!__rap__context__[key]) {")
							.append("		__rap__context__[key] = {").append("			arr : orderArr,")
							.append("			index : 0").append("		};").append("	}")
							.append("	return geneVal(key);").append("}");

					return orderCmdFunc.toString();
				}
			} else if (mockValue.startsWith("@order")) {
				return "\"" + StringUtils.escapeInJ(mockValue) + "\"";
			} else if ((para.getDataType().equals("number") || para.getDataType().equals("boolean"))
					&& !mockValue.startsWith("@")) {
				return mockValue;
			} else if ((para.getDataType().equals("array<number>") || para.getDataType().equals("array<boolean>")
					|| para.getDataType().equals("array<string>"))) {
				String result = mockValue;
				if (para.getDataType().equals("array<string>") && !result.startsWith("\"") && !result.startsWith("'")) {
					result = "\"" + result + "\"";
				}
				return "[" + result + "]";
			} else {
		...
		} else if (mockValue != null && mockValue.isEmpty() && para.getDataType().equals("string")) {
			return "\"\"";
			...
			if (tag.startsWith("@") && tag.contains("=")) {
				...
				if (tag.startsWith("@value")) {
					...
					if (tag.contains("value[")) {
						tagMap.put("value_index",
								val + "_INDEX_" + tag.substring(tag.indexOf("[") + 1, tag.indexOf("]")));
					} else {
	...
	public int modify(int actionId, String mockData) {
		...
		for (String snip : mockDataSnips) {
			Set<Parameter> pList = snip.startsWith("request.") ? action.getRequestParameterList()
	...
	private Parameter locateParam(Set<Parameter> pList, String snip) {
		boolean hasSubParam = snip.indexOf(".") > 0 && snip.indexOf(".") < snip.indexOf("=");
		String identifier = hasSubParam ? snip.substring(0, snip.indexOf(".")) : snip.substring(0, snip.indexOf("="));
		for (Parameter p : pList) {
			if (p.getIdentifier().equals(identifier)) {
				return hasSubParam ? locateParam(p.getParameterList(), snip.substring(snip.indexOf(".") + 1)) : p;
			}
		}
		return null;
	}
